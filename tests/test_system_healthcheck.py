...
def test_canary_latency_only_red_is_relaxed_to_yellow_in_dev(monkeypatch):
    class _FakeCanary:
        ...
        def assess(self, summary=None, mode="dev"):
            return system_healthcheck.CanaryAssessment(
                status="RED",
                summary="Canary source health is below threshold.",
                warnings=[],
                errors=["p95_latency_ms_1h=3074.81 > 3000.0"],
                evidence=[],
                windows={},
            )
    ...
    assert any("Latency-only canary degradation" in msg for msg in out.warnings)
    assert not out.errors


def test_canary_non_latency_red_stays_red_in_dev(monkeypatch):
    class _FakeCanary:
        def collect_once(self):
            return None
        ...
        def assess(self, summary=None, mode="dev"):
            return system_healthcheck.CanaryAssessment(
                ...
                summary="Canary source health is below threshold.",
                warnings=[],
                errors=["success_rate_1h=0.8 < 0.95"],
                evidence=[],
                windows={},
            )
    ...
    assert out.status == "RED"
    assert out.errors == ["success_rate_1h=0.8 < 0.95"]
