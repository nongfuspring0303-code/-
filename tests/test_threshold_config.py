...
KNOWN_LEGACY_HARDCODED = {
    "conduction_mapper.py": {
        711: "max_recommended_high_conf",
        712: "max_recommended_mid_conf",
        713: "max_recommended_low_conf",
        1003: "template_base_confidence",
        1005: "template_rate_cut_confidence",
        1007: "template_inflation_confidence",
        1065: "tariff_asset_confidence",
        1091: "tariff_confidence",
        1114: "high_threshold",
        1115: "medium_threshold",
        1161: "max_evidence_items",
        1396: "policy_base_confidence",
        1400: "policy_confidence_min",
        1401: "policy_confidence_max",
        1402: "policy_confidence_scale",
        1403: "policy_confidence_base",
        1413: "policy_asset_confidence",
        1478: "fallback_confidence",
    },
...
def test_feature_flags_impl3_resolver_merge_flags_exist():
    """PR-3 support flags must exist and stay disabled by default until runtime lands."""
    config_path = os.path.join(REPO_ROOT, "configs", "feature_flags_v22.yaml")
    with open(config_path) as f:
        cfg = _yaml.safe_load(f)
    flags = cfg.get("flags", {})
    ...
    required = {
        "enable_entity_resolver": "member_b",
        "enable_unified_candidate_pool": "member_b",
        "enable_multisource_merge": "member_b",
    }
    missing = [name for name in required if name not in flags]
    assert not missing, f"Required Stage8A Impl-3 flags missing from config: {missing}"

    invalid = []
    for name, owner in required.items():
        flag = flags.get(name, {})
        if flag.get("default") is not False:
            invalid.append(f"{name}.default must be false before Impl-3 runtime lands")
        if flag.get("owner") != owner:
            invalid.append(f"{name}.owner must be {owner}")
        if flag.get("rollback_owner") != owner:
            invalid.append(f"{name}.rollback_owner must be {owner}")
    ...
    assert not invalid, "Invalid Stage8A Impl-3 feature flag metadata:\n" + "\n".join(invalid)


def test_feature_flags_impl4_phase3_flags_exist():
    """Phase 3 support flags must exist and stay disabled by default until runtime lands."""
    config_path = os.path.join(REPO_ROOT, "configs", "feature_flags_v22.yaml")
    with open(config_path) as f:
        cfg = _yaml.safe_load(f)
    flags = cfg.get("flags", {})
    ...
    required = {
        "enable_semantic_full_peer_expansion": "member_b",
        "enable_market_validation_gate": "member_b",
    }
    missing = [name for name in required if name not in flags]
    assert not missing, f"Required Stage8A Phase 3 flags missing from config: {missing}"

    invalid = []
    for name, owner in required.items():
        flag = flags.get(name, {})
        if flag.get("default") is not False:
            invalid.append(f"{name}.default must be false before Phase 3 runtime lands")
        if flag.get("owner") != owner:
            invalid.append(f"{name}.owner must be {owner}")
        if flag.get("rollback_owner") != owner:
            invalid.append(f"{name}.rollback_owner must be {owner}")
